...
perex: Dnes vám trochu přiblížím výbavu mého počítače, hlavně tu softwarovou. Na počítači primárně programuji, ale i hraji a relaxuji. Pracovním strojem je 14ti palcový Dell Latitude z roku 2019...
---

<sidenote class="left">**AI Shrnutí:** Přehled softwarové výbavy vývojářského notebooku Dell Latitude s Linux Manjaro KDE. Klíčové nástroje: PHPStorm, Docker, KeepassXC pro správu hesel a SSH klíčů, Google Chrome, OneDrive přes Onedriver. Zálohy řešené pomocí Timeshift (systém) a Kopia UI (data do AWS S3). Kompletní setup pro webový vývoj s důrazem na bezpečnost a zálohování.</sidenote> Dnes vám trochu přiblížím výbavu měho počítače, hlavně tedy tu softwarovou výbavu. Na počítači primárně programuji, ale i hraji a relaxuji. Můj pracovní stroj je 14ti palcový notebook Dell Latitude z roku 2019. Procesor je Intel i7 s 6
...
Manjaro <sidenote>Manjaro je uživatelsky přívětivá distribuce Linuxu založená na Arch Linuxu s přístupem k AUR (Arch User Repository).</sidenote>. Zprvu na variantu XFCE a pote na KDE a spokojeně jej uživám dodnes. Co se ale v nedávné době změnilo je
dualboot s Windows. Ten jsem již zrušil a mám tedy pouze Linux. Na hry používám už jen Geforce Now <sidenote>GeForce Now je cloudová herní služba NVIDIA, která umožňuje streamovat hry bez nutnosti výkonného hardware.</sidenote> v kombinaci se
...
zvolil moderní Btrfs <sidenote>Btrfs je moderní souborový systém s pokročilými funkcemi jako snapshoty, komprese a kontrola integrity dat.</sidenote>, který má výhodu v rychlém vytváření snapshotů, kterou využijemi dále při nastavení zálohování.
...
Pak je potřeba zapnout baličky i z AUR <sidenote>AUR (Arch User Repository) obsahuje balíčky spravované komunitou, včetně proprietárního softwaru nedostupného v oficiálních repozitářích.</sidenote>, kde je spousta proprietárního softwaru.

Ještě jedna důležitá úprava - zvětšení velikosti swapu <sidenote>Swap je virtuální paměť na disku, která se používá když dojde fyzická RAM. 20 GB je hodně, ale užitečné pro náročné vývojářské úlohy.</sidenote>. Při instalaci jsem zvolil swapování do souboru, takže
...
Důležitá data, včetně klíčenky na hesla mám dlouhá léta na Microsoft OneDrive. Onedriver <sidenote>Onedriver je neoficiální Linux klient pro Microsoft OneDrive, který umožňuje připojit cloudové úložiště jako lokální disk.</sidenote> dokáže připojit OneDrive
...
prohlížeči, ssh a jiných aplikacích. Také je to uložiště pro mé ssh klíče <sidenote>KeePassXC může spravovat SSH klíče a integrovat se s SSH agentem, což eliminuje nutnost zadávat hesla při každém připojení.</sidenote>. Samozřejmostí je zapnutí automatického
...
a zadal velmi silné heslo, například 120 znaků dlouhé a vygenerované v Keepassu <sidenote>Ed25519 je moderní a bezpečný algoritmus pro SSH klíče, rychlejší a bezpečnější než starší RSA.</sidenote>.
...
Pro zálohu home adresáře. Velká výhoda je, že lze zálohy ukládat do AWS S3 <sidenote>AWS S3 je cloudové úložiště s vysokou dostupností a durability, ideální pro dlouhodobé uchovávání záloh.</sidenote>. Protože si se službami AWS tykám, je to
...
Často využívaný Docker a Docker Compose <sidenote>Docker umožňuje kontejnerizaci aplikací pro konzistentní prostředí napříč různými systémy. Docker Compose orchestruje více kontejnerů.</sidenote>, svatý grál současného vývoje. Pro spuštění je opět potřeba trochu magie
